  ...
  <div>
    <Sortable
      ...
      <template #item="{element: image, index}">
        <div
            class="w-[120px] h-[120px] rounded border border-dashed flex items-center justify-center relative hover:border-purple-500 draggable" :key="image.id">
    ...
    <div
      class="relative w-full h-[40px] rounded border border-gray-400 border-dashed flex items-center justify-center hover:border-purple-500 overflow-hidden mt-3">
        <span>
          Upload
        </span>
...
const emit = defineEmits(['update:modelValue', 'update:deletedImages', 'update:imagePositions', 'reorder'])
...
function onImageDragEnd(ev){
  ...
  const oldImage = files.value[oldIndex]

  files.value.splice(newIndex)
}
// Hooks
watch('props.images', () => {
  imageUrls.value = [
